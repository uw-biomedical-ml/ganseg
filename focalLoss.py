...
class FocalLoss(torch.nn.Module):
    ...
    def forward(self, input, target):
        # print('focal', input.shape, target.shape)
...
class NormalizedFocalLoss(torch.nn.Module):
    ...
    def forward(self, input, target):
        print('normFocal', input.shape, target.shape)

        # copy from FocalLoss above to ensure shape alignment
        if input.dim() > 2:
            input = input.view(input.size(0), input.size(1), -1)  # N,C,H,W => N,C,H*W
            input = input.transpose(1, 2)                         # N,C,H*W => N,H*W,C
            input = input.contiguous().view(-1, input.size(2))    # N,H*W,C => N*H*W,C
...
@mlconfig.register
class NormalizedFocalLossWeighted(torch.nn.Module):
    def __init__(self, scale=1.0, gamma=0, num_classes=10, alpha=None, size_average=True, weights=1):
        super(NormalizedFocalLossWeighted, self).__init__()
        self.gamma = gamma
        self.size_average = size_average
        self.num_classes = num_classes
        self.scale = scale
    ...
    def forward(self, input, target):
        print('normFocalWeighted', input.shape, target.shape)
        ...
        logpt_weighted = logpt * self.weights
        normalizor = torch.sum(-1 * (1 - logpt_weighted.data.exp()) ** self.gamma * logpt_weighted , dim=1)
        logpt_weighted = logpt_weighted.gather(1, target)
        logpt_weighted = logpt_weighted.view(-1)
        pt = torch.autograd.Variable(logpt_weighted.data.exp())

        loss = -1 * (1-pt)**self.gamma * logpt_weighted
        loss = self.scale * loss / normalizor
